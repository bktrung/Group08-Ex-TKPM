...
export const deleteDepartmentValidator = (req: Request, res: Response, next: NextFunction) => {
	const { id } = req.params;
	...
	if (!/^[0-9a-fA-F]{24}$/.test(id)) {
		return next(new BadRequestError('Department ID must be a valid ObjectId'));
	}
